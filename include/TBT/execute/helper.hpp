...
namespace TBT {
  ...
  namespace Execute {
    ...
    inline States::NodeHeader read_node_header(const int8_t* _node) {
    ...
    inline States::Header read_global_node_header(const int8_t* _node) {
    ...
    template <class T>
    T read_node_state(const int8_t* _node) {
    ...
    inline uint32_t read_root_child(const int32_t& _i, const int8_t* _node) {
      return read<uint32_t>(_node + sizeof(States::Header) + _i * sizeof(uint32_t));
    }  // read_child

    inline uint32_t read_child(const int32_t& _i, const States::NodeHeader& _header, const int8_t* _node) {
    ...
    [[nodiscard]] inline Parameter read_payload(const int32_t& _i, const States::NodeHeader& _header,
                                                const int8_t* _node) {
      constexpr int32_t size    = sizeof(char) + sizeof(int32_t);

      const uint8_t type        = read<uint8_t>(_node + _header.params_offset_ + _i * size);
      const Detail::Payload val = read<Detail::Payload>(_node + _header.params_offset_ + _i * size + sizeof(char));
