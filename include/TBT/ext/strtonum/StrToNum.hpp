  ...
  namespace impl {
    ...
    inline constexpr void* memmove_constexpr(std::uint32_t* pDest, const std::uint32_t* pSrc, std::size_t sSize) {
      if (std::is_constant_evaluated()) {
        ...
        while (sSizeUint32-- > 0) { *pDest++ = *pSrc++; }
        return pDest;
      }
    ...
    [[nodiscard]] constexpr from_chars_result<CharT> Integer_from_chars(const CharT* const First,
                                                                        const CharT* const Last, RawTy& Raw_value,
                                                                        int Base) noexcept {
      ...
      if (Base == 0) {
        ...
        if (HasHexPrefix(Next, Last)) { Next += 2; }
      ...
      if (Next - First == static_cast<ptrdiff_t>(Minus_sign)) { return {First, errc::invalid_argument}; }

      if (Overflowed) { return {Next, errc::result_out_of_range}; }

      if constexpr (std::is_signed_v<RawTy>) {
        if (Minus_sign) { Value = static_cast<Unsigned>(0 - Value); }
      }
    ...
    struct Big_integer_flt {
      ...
      constexpr Big_integer_flt(const Big_integer_flt& Other) noexcept : Myused(Other.Myused) {
        if (std::is_constant_evaluated()) { std::fill_n(Mydata, Element_count, 0); }
      ...
      [[nodiscard]] constexpr bool operator<(const Big_integer_flt& Rhs) const noexcept {
        if (Myused != Rhs.Myused) { return Myused < Rhs.Myused; }

        for (uint32_t Ix = Myused - 1; Ix != static_cast<uint32_t>(-1); --Ix) {
          if (Mydata[Ix] != Rhs.Mydata[Ix]) { return Mydata[Ix] < Rhs.Mydata[Ix]; }
        }
    ...
    [[nodiscard]] inline constexpr uint32_t Bit_scan_reverse(const uint32_t Value) noexcept {
      ...
      if (BitScanReverse_constexpr(&Index, Value)) { return Index + 1; }
...
#ifdef WIN64
      if (BitScanReverse64(&Index, Value)) { return Index + 1; }
#else   // ^^^ 64-bit ^^^ / vvv 32-bit vvv
      uint32_t Ui32 = static_cast<uint32_t>(Value >> 32);

      if (BitScanReverse_constexpr(&Index, Ui32)) { return Index + 1 + 32; }

      Ui32 = static_cast<uint32_t>(Value);

      if (BitScanReverse_constexpr(&Index, Ui32)) { return Index + 1; }
#endif  // ^^^ 32-bit ^^^
    ...
    [[nodiscard]] inline constexpr uint32_t Add_carry(uint32_t& Ux1, const uint32_t Ux2,
    ...
    [[nodiscard]] inline constexpr uint32_t Add_multiply_carry(uint32_t& U_add, const uint32_t U_mul_1,
                                                               const uint32_t U_mul_2,
                                                               const uint32_t U_carry) noexcept {
    ...
    [[nodiscard]] inline constexpr uint32_t Multiply_core(uint32_t* const Multiplicand,
                                                          const uint32_t Multiplicand_count,
    ...
    [[nodiscard]] inline constexpr bool Multiply(Big_integer_flt& Multiplicand, const uint32_t Multiplier) noexcept {
      ...
      if (Multiplicand.Myused == 0) { return true; }
    ...
    [[nodiscard]] inline constexpr bool Multiply(Big_integer_flt& Multiplicand,
                                                 const Big_integer_flt& Multiplier) noexcept {
      if (Multiplicand.Myused == 0) { return true; }
    ...
    inline constexpr uint32_t Large_power_data[] = {
    ...
    inline constexpr Unpack_index Large_power_indices[] = {
    ...
    inline constexpr uint32_t Small_powers_of_ten[9] = {10,        100,        1'000,       10'000,       100'000,
    ...
    [[nodiscard]] inline constexpr uint64_t Divide(Big_integer_flt& Numerator,
                                                   const Big_integer_flt& Denominator) noexcept {
      // If the Numerator is zero, then both the quotient and remainder are zero:
      if (Numerator.Myused == 0) { return 0; }
      ...
      if (Max_denominator_element_index > Max_numerator_element_index) { return 0; }
      ...
      for (int32_t Iu = static_cast<int32_t>(Max_numerator_element_index);; --Iu) {
        ...
        if (Denominator.Mydata[Iu - Cu_diff] != Numerator.Mydata[Iu]) {
          if (Denominator.Mydata[Iu - Cu_diff] < Numerator.Mydata[Iu]) { ++Cu_quo; }

          break;
        }
      }

      if (Cu_quo == 0) { return 0; }
      ...
      if (Cbit_shift_left > 0) {
        ...
        if (Cu_den > 2) { U_den_next |= Denominator.Mydata[Cu_den - 3] >> Cbit_shift_right; }
      }
      ...
      for (int32_t Iu = Cu_quo; --Iu >= 0;) {
        ...
        if (Cbit_shift_left > 0) {
          ...
          if (Iu + Cu_den >= 3) { U_num_next |= Numerator.Mydata[Iu + Cu_den - 3] >> Cbit_shift_right; }
        }
        ...
        if (Uu_quo > 0) {
          ...
          for (uint32_t Iu2 = 0; Iu2 < Cu_den; ++Iu2) {
            ...
            Uu_borrow >>= 32;
            if (Numerator.Mydata[Iu + Iu2] < U_sub) { ++Uu_borrow; }
      ...
      // Trim the remainder:
      for (uint32_t Ix = Max_numerator_element_index + 1; Ix < Numerator.Myused; ++Ix) { Numerator.Mydata[Ix] = 0; }

      uint32_t Used = Max_numerator_element_index + 1;

      while (Used != 0 && Numerator.Mydata[Used - 1] == 0) { --Used; }
    ...
    [[nodiscard]] inline bool Should_round_up(const bool Lsb_bit, const bool Round_bit,
                                              const bool Has_tail_bits) noexcept {
      return Round_bit && (Has_tail_bits || Lsb_bit);
    }

    [[nodiscard]] inline constexpr uint64_t Right_shift_with_rounding(const uint64_t Value, const uint32_t Shift,
    ...
    template <class FloatingType>
    constexpr void Assemble_floating_point_value_no_shift(
        const bool Is_negative, const int32_t Exponent,
        const typename Floating_type_traits<FloatingType>::Uint_type Mantissa, FloatingType& Result) noexcept {
      // The following code assembles floating-point values based on an alternative interpretation of the IEEE 754
      // binary floating-point format. It is valid for all of the following cases: [1] normal value, [2] normal value,
      // needs renormalization and exponent increment after rounding up the mantissa, [3] normal value, overflows after
      // rounding up the mantissa, [4] subnormal value, [5] subnormal value, becomes a normal value after rounding up
      // the mantissa.
    ...
    template <class FloatingType>
    [[nodiscard]] constexpr errc Assemble_floating_point_value(const uint64_t Initial_mantissa,
                                                               const int32_t Initial_exponent, const bool Is_negative,
    ...
    inline constexpr void Accumulate_decimal_digits_into_big_integer_flt(const uint8_t* const First_digit,
    ...
    template <class FloatingType>
    [[nodiscard]] constexpr errc Convert_decimal_string_to_floating_type(const Floating_point_string& Data,
                                                                         FloatingType& Result,
      ...
      // range [1.0, 2.0). This will either be the same as the shift we computed earlier, or one greater than that
      // shift:
      ...
      return Assemble_floating_point_value(Complete_mantissa, Final_exponent, Data.Myis_negative, Has_zero_tail,
                                           Result);
    }

    template <class FloatingType>
    [[nodiscard]] constexpr errc Convert_hexadecimal_string_to_floating_type(const Floating_point_string& Data,
                                                                             FloatingType& Result,
                                                                             bool Has_zero_tail) noexcept {
      using Traits                       = Floating_type_traits<FloatingType>;
      ...
      while (Has_zero_tail && Mantissa_it != Mantissa_last) { Has_zero_tail = *Mantissa_it++ == 0; }
    ...
    [[nodiscard]] constexpr from_chars_result<CharT> Ordinary_floating_from_chars(
        const CharT* const First, const CharT* const Last, Floating& Value, const chars_format Fmt,
      ...
      if (Is_hexadecimal && HasHexPrefix(Next, Last)) { Next += 2; }
      ...
      if (Whole_begin == Whole_end && Dot_end == Frac_end) { return {First, errc::invalid_argument}; }
      ...
      if (Fmt != chars_format::fixed  // N4713 23.20.3 [charconv.from.chars]/7.3
        ...
        if (Exponent_is_negative) { Exponent = -Exponent; }
      }
      ...
      while (Mantissa_it != Mantissa_first && *(Mantissa_it - 1) == 0) { --Mantissa_it; }

      // If the mantissa buffer is empty, the mantissa was composed of all zeroes (so the mantissa is 0).
      // All such strings have the value zero, regardless of what the exponent is (because 0 * b^n == 0 for all b and
      // n). We can return now. Note that we defer this check until after we scan the exponent, so that we can correctly
    ...
    template <class CharT>
    [[nodiscard]] inline bool Starts_with_case_insensitive(const CharT* First, const CharT* const Last,
                                                           const char* Lowercase) noexcept {
      // pre: Lowercase contains only ['a', 'z'] and is null-terminated
      for (; First != Last && *Lowercase != '\0'; ++First, ++Lowercase) {
        if ((static_cast<unsigned char>(*First) | 0x20) != *Lowercase) { return false; }
      }
    ...
    template <class Floating, class CharT>
    [[nodiscard]] constexpr from_chars_result<CharT> Infinity_from_chars(const CharT* const First,
                                                                         const CharT* const Last, Floating& Value,
                                                                         const bool Minus_sign,
                                                                         const CharT* Next) noexcept {
    ...
    template <class Floating, class CharT>
    [[nodiscard]] constexpr from_chars_result<CharT> Nan_from_chars(const CharT* const First, const CharT* const Last,
                                                                    Floating& Value, bool Minus_sign,
                                                                    const CharT* Next) noexcept {
      ...
      if (Next != Last && *Next == '(') {  // possibly nan(n-char-sequence[opt])
        ...
        for (const auto* Temp = Seq_begin; Temp != Last; ++Temp) {
          if (*Temp == ')') {  // definitely nan(n-char-sequence[opt])
            ...
            if (Temp - Seq_begin == 3 && Starts_with_case_insensitive(Seq_begin, Temp, "ind")) {  // definitely nan(ind)
      ...
      if (Minus_sign) { Uint_value |= Traits::Shifted_sign_mask; }
    ...
    template <class Floating, class CharT>
    [[nodiscard]] constexpr from_chars_result<CharT> Floating_from_chars(const CharT* const First,
                                                                         const CharT* const Last, Floating& Value,
      ...
      if (Next == Last) { return {First, errc::invalid_argument}; }
