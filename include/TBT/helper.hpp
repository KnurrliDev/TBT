...
namespace TBT {
  ...
  using TaskQueue = std::forward_list<std::pair<std::function<TBT::State()>, std::promise<TBT::State>>>;
...
#define COMPILE_AND_QUEUE(tree, states, ...)                                                        \
  [&]() -> auto {                                                                                   \
    ...
    auto& ref                   = states.tasks_queue_.emplace_front(std::move(ct), std::move(prm)); \
