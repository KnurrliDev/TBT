...
namespace stn::impl {
  ...
  [[nodiscard]] constexpr from_chars_result<CharT> Integer_from_chars(const CharT* const First, const CharT* const Last,
                                                                      RawTy& Raw_value, int Base) noexcept {
  ...
  [[nodiscard]] constexpr uint32_t Add_multiply_carry(uint32_t& U_add, const uint32_t U_mul_1, const uint32_t U_mul_2,
  ...
  [[nodiscard]] constexpr uint32_t Multiply_core(uint32_t* const Multiplicand, const uint32_t Multiplicand_count,
  ...
  constexpr uint32_t Large_power_data[] = {
  ...
  constexpr Unpack_index Large_power_indices[] = {
  ...
  constexpr uint32_t Small_powers_of_ten[9] = {10,        100,        1'000,       10'000,       100'000,
  ...
  [[nodiscard]] bool Should_round_up(const bool Lsb_bit, const bool Round_bit, const bool Has_tail_bits) noexcept {
    return Round_bit && (Has_tail_bits || Lsb_bit);
  }

  [[nodiscard]] constexpr uint64_t Right_shift_with_rounding(const uint64_t Value, const uint32_t Shift,
  ...
  [[nodiscard]] constexpr errc Assemble_floating_point_value(const uint64_t Initial_mantissa,
                                                             ...
                                                             const bool Has_zero_tail, FloatingType& Result) noexcept {
  ...
  constexpr void Accumulate_decimal_digits_into_big_integer_flt(const uint8_t* const First_digit,
  ...
  [[nodiscard]] constexpr from_chars_result<CharT> Ordinary_floating_from_chars(const CharT* const First,
  ...
  template <class CharT>
  [[nodiscard]] bool Starts_with_case_insensitive(const CharT* First, const CharT* const Last,
